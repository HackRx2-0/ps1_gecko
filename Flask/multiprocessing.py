...
class QueryProcessor:
    def __init__(self):
        try:
            self.summarizer = joblib.load("/content/drive/MyDrive/Hackathon/Models/summarizer.joblib")
            self.nlp_qa = joblib.load("/content/drive/MyDrive/Hackathon/Models/question_answering.joblib")
            self.model = TFBertModel.from_pretrained("/content/drive/MyDrive/Hackathon/Models/Bert_model")
            self.tokenizer = joblib.load("/content/drive/MyDrive/Hackathon/Models/Bert_tokenizer.joblib")
...
def do_search(i,data,query_keywords):
    try:
        if Q.fuzz_matching(query_keywords,eval(str(data["keywords"].values[i])))>30:
            answer = Q.question_answering(data["processed_blog"].values[i],query)
            print({"Answer":answer,"Summary":data["summary"].values[i],"URL":data["link"].values[i],"keywords":query_keywords})
    except:
        pass

def search(query):
    query_keywords = Q.keyword_extraction(query)
    count = 1
    lol = []
    for i in range(len(data)):
        x = multiprocessing.Process(target = do_search,args=(i,data,query_keywords))
        lol.append(x)
        x.start()
    for j in lol:
        j.join()
